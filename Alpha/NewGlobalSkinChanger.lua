...
function LoadMenu()
	do
		...
		SkinChanger.Settings:Menu("My Hero", "me")
		SkinChanger.Settings.me:DropDown("me", "Change "..myHero.charName.." Skin", 2, SkinNames[myHero.charName], function(Val) SetSkin(myHero, Val - 1) end)
		SkinChanger.Settings:Menu("Enemies", "enemy", "leaf")
		SkinChanger.Settings:Menu("Allies", "ally", "gamepad")
		for i = 1,6 do
			if GetEnemyHeroes()[i] ~= nil then
				SkinChanger.Settings.enemy:DropDown("EnemyNumber"..i, "Change "..GetEnemyHeroes()[i].charName.." Skin", 2, SkinNames[GetEnemyHeroes()[i].charName], function(Val) SetSkin(GetEnemyHeroes()[i], Val - 1) end)
			end
			if GetAllyHeroes()[i] ~= nil then
				SkinChanger.Settings.ally:DropDown("AllyNumber"..i, "Change "..GetAllyHeroes()[i].charName.." Skin", 2, SkinNames[GetAllyHeroes()[i].charName], function(Val) SetSkin(GetAllyHeroes()[i], Val - 1) end)
			end
		end
	end
end
